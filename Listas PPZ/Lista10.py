...
def sum13(nums):
  if 13 in nums:
    return sum(nums[:nums.index(13)])     # ele vai fazer a soma até chegar na posição em que o index é igual a 13 e parar
  return sum(nums)
...
# has22([2, 1, 2]) -> False
def has22(nums):
  return '2, 2' in str(nums)  # ele pega o 2,2 e lê como se fosse string na lista
